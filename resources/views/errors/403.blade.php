...
@endphp
<x-header title="403 - Forbidden | Laboratory Management System" />

<body class="font-sans antialiased bg-mainBg text-gray-800 flex items-center justify-center min-h-screen p-4 sm:p-6 relative overflow-hidden selection:bg-rose-500 selection:text-white">

    <!-- Ambient background glowing orbs -->
    <div class="absolute top-[-10%] left-[-10%] w-96 h-96 bg-rose-500 rounded-full mix-blend-multiply filter blur-[128px] opacity-20 animate-pulse"></div>
    <div class="absolute bottom-[-10%] right-[-10%] w-96 h-96 bg-amber-500 rounded-full mix-blend-multiply filter blur-[128px] opacity-15 animate-pulse" style="animation-delay: 2s;"></div>
    ...
    <div class="w-full max-w-lg bg-white/90 backdrop-blur-xl border border-gray-200/80 rounded-3xl p-6 sm:p-10 shadow-xl relative z-10 text-center transition-all">
        ...
        <!-- Status Badge -->
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rose-50 border border-rose-200 text-rose-600 text-xs font-bold tracking-wider uppercase mb-6">
        ...
        <div class="relative w-24 h-24 mx-auto mb-6 flex items-center justify-center">
            <div class="absolute inset-0 rounded-2xl bg-rose-500/10 border border-rose-500/20 rotate-6 transition-transform hover:rotate-12 duration-300"></div>
            <div class="relative w-20 h-20 bg-rose-600 text-white rounded-2xl shadow-lg flex items-center justify-center neo-shadow-sm">
                <i class="ph-duotone ph-shield-warning text-4xl"></i>
            </div>
        </div>

        <!-- Title and Description -->
        <h1 class="text-6xl sm:text-7xl font-black text-gray-900 tracking-tight mb-2">403</h1>
        <h2 class="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight mb-3">Permission Denied</h2>
        ...
        <div class="bg-gray-50 border border-gray-200/80 rounded-2xl p-4 mb-6 text-left shadow-2xs">
            <div class="flex items-center justify-between text-xs font-semibold text-gray-600 mb-2">
                <span class="flex items-center gap-2">
                    <i class="ph-duotone ph-arrows-clockwise text-base text-rose-500 animate-spin" style="animation-duration: 3s;"></i>
            ...
            <div class="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden">
                <div id="countdown-bar" class="bg-rose-500 h-1.5 rounded-full transition-all duration-1000 ease-linear" style="width: 100%;"></div>
            </div>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (toggleBtn) {
                toggleBtn.addEventListener('click', () => {
                    isPaused = !isPaused;
                    if (isPaused) {
                        if (pauseIcon) pauseIcon.className = "ph-bold ph-play text-base text-rose-600";
                        if (pauseText) pauseText.textContent = "Resume";
                        toggleBtn.classList.add('border-rose-300', 'bg-rose-50');
                    } else {
                        if (pauseIcon) pauseIcon.className = "ph-bold ph-pause text-base text-gray-500";
                        if (pauseText) pauseText.textContent = "Pause";
                        toggleBtn.classList.remove('border-rose-300', 'bg-rose-50');
                    }
                });
            }
        });
