document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateUserBackdrop = document.getElementById('UpdateUserModalBackdrop');
        ...
        setTimeout(() => {
            ...
            if (formId) {
                ...
                if (formId === 'AddUserForm') {
                    document.getElementById('addUserImagePreview').classList.add('hidden');
                    document.getElementById('addUserImagePreview').src = '#';
                    document.getElementById('addUserCameraIcon').classList.remove('hidden');
                }
            }
        }, 300);
    }
    ...
    function clearUserErrors(prefix) {
        document.querySelectorAll(`[id^="error${prefix === 'add' ? 'Add' : 'Update'}User"]`).forEach(el => {
            el.classList.add('hidden');
            el.innerText = '';
        });
        document.querySelectorAll(`#${prefix === 'add' ? 'Add' : 'Update'}UserForm input, #${prefix === 'add' ? 'Add' : 'Update'}UserForm select`).forEach(el => {
    ...
    document.getElementById('CloseAddUserX').addEventListener('click', () => closeModal(addUserBackdrop, addUserModal, 'AddUserForm'));
    document.getElementById('CloseAddUserBtn').addEventListener('click', () => closeModal(addUserBackdrop, addUserModal, 'AddUserForm'));
    document.getElementById('CloseUpdateUserX').addEventListener('click', () => closeModal(updateUserBackdrop, updateUserModal, 'UpdateUserForm'));
    document.getElementById('CloseUpdateUserBtn').addEventListener('click', () => closeModal(updateUserBackdrop, updateUserModal, 'UpdateUserForm'));
    ...
    function fetchDepartments() {
        fetch('/departments', { headers: { 'Accept': 'application/json' } })
            ...
            .then(data => {
                ...
                const updateDept = document.getElementById('updateUserDepartment');
                addDept.innerHTML = '<option value="">None (General)</option>';
                updateDept.innerHTML = '<option value="">None (General)</option>';
                if (data.data) {
        ...
        fetch('/allusers', { headers: { 'Accept': 'application/json' } })
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    renderUserGrid('grid-pathologist', users.filter(u => u.role === 'Pathologist'));
                    renderUserGrid('grid-sample-collector', users.filter(u => u.role === 'SampleCollector'));
                    renderUserGrid('grid-technician', users.filter(u => u.role === 'Technician'));
                    renderUserGrid('grid-specialist', users.filter(u => u.role === 'SpecialistDoctor'));
                    renderUserGrid('grid-receptionist', users.filter(u => u.role === 'Receptionist'));
                }
            });
    }
    ...
    function renderUserGrid(containerId, users) {
        ...
        container.innerHTML = users.map(user => {
            ...
            const deptName = user.department ? user.department.name : 'General';
    ...
    window.editUser = async function(id) {
        try {
            const response = await fetch(`/user/${id}`, { headers: { 'Accept': 'application/json' } });
            ...
            if (data.status === 'success') {
                ...
                document.getElementById('updateUserDepartment').value = user.department_id || '';
                document.getElementById('updateUserPassword').value = '';
                
                const previewEl = document.getElementById('updateUserImagePreview');
        ...
        try {
            ...
            if (response.ok) {
                if (card) {
                    ...
                    setTimeout(() => fetchAndPopulateUsers(), 300);
                } else {
                    fetchAndPopulateUsers();
                }
            }
    ...
    const updateImagePreview = document.getElementById('updateUserImagePreview');
    ...
    const updatePasswordToggle = document.getElementById('updateUserPasswordToggle');
    ...
    if (updateImageCircle) {
        updateImageCircle.addEventListener('click', () => updateImageInput.click());
    }
    ...
    if (addImageInput) handleImagePreview(addImageInput, addImagePreview, addCameraIcon);
    if (updateImageInput) handleImagePreview(updateImageInput, updateImagePreview, updateCameraIcon);
    ...
    if (addPasswordToggle) handlePasswordToggle(addPasswordToggle, addPasswordInput, addEyeIcon);
    if (updatePasswordToggle) handlePasswordToggle(updatePasswordToggle, updatePasswordInput, updateEyeIcon);
});